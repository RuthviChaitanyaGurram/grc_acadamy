...
function loadScriptFile(path) {
  ...
  const candidatePaths = [path, basename, `data/topics/${basename}`, `topics/${basename}`];

  return new Promise((resolve, reject) => {
    const tryLoad = (index) => {
      if (index >= candidatePaths.length) {
        reject(new Error(`Failed to load topic file (${path}).`));
        return;
      }
      const script = document.createElement("script");
      script.src = candidatePaths[index];
...
function loadManifestOnlyMode() {
  ...
  if (total === 0) {
    dataStatus.textContent = "Data mode: Manifest not found. Upload manifest.js (or data/manifest.js) and refresh.";
    return;
  }
